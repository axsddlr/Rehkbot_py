...
class Webhook:
    ...
    CDN = (
        r"https://cdn.discordapp.com/avatars/" r"{0.id}/{0.default_avatar}.{1}?size={2}"
    )
    ...
    def __init__(
        ...
        **options
        ...
        if not self.url and (self.id == -1 or not self.token):
            ...
            raise ValueError("url and (id or token) must not be both " "provided.")
        ...
        if session is not None:
            ...
            if is_async and not isinstance(self.session, aiohttp.ClientSession):
                raise TypeError(
                    "is_async is set to True, but session "
                    "isn't aiohttp.ClientSession."
                )
            elif not is_async and not isinstance(self.session, requests.Session):
                raise TypeError(
                    "is_async is set to False, but session " "isn't requests.Session."
                )
    ...
    def send(
        ...
        if not content and not embeds and not file:
            raise ValueError("One of content, embed/embeds, " "or file must be set")
        payload["embeds"] = [em.to_dict() for em in embeds]

        return self._request("POST", payload, file=file)

    @alias("edit")
    def modify(self, name: str = "", avatar: bytes = b"") -> "Webhook":
        """Edits the webhook.
            ...
            if method == "POST":
                if file is not None:
                    ...
                    resp = self.session.post(
                        self.url, data=payload, headers=headers, files=multipart
                    )
                else:
                ...
                resp = self.session.patch(self.url, json=payload, headers=headers)
        ...
        while rate_limited:
            if method == "POST":
                if file is not None:
                    ...
                    resp = await self.session.post(self.url, data=data, headers=headers)
                ...
                resp = await self.session.delete(self.url, headers=headers)

            elif method == "PATCH":
                resp = await self.session.patch(self.url, json=payload, headers=headers)
