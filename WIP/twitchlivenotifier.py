...
def config():
    ...
    try:
        ...
        print(
            "ImagePriority not found in Twitch section of config file. "
    ...
    try:
        twitch_client_id = C_ID
    except KeyError:
        print(
            "ClientId not found in Twitch section of config file. Please set ClientId under [Twitch] in config.ini"
        )
        print(
            "This is the Client ID you receive when registering an application as a Twitch developer."
        )
    ...
    try:
        twitch_secret_key = C_SEC
    except KeyError:
        print(
            "ClientSecret not found in Twitch section of config file. Please set ClientSecret under [Twitch] in "
            "config.ini"
        )
        print(
            "This is the Client Secret you receive when registering an application as a Twitch developer."
        )
        print("Please check the README for more instructions.")
        sys.exit()
    ...
    try:
        ...
        print(
            "Url not found in Discord section of config file. Please set Url under [Discord] in config.ini"
        )
        print(
            "This can be found by editing a Discord channel, selecting Webhooks, and creating a hook."
        )
        sys.exit()
    ...
    try:
        ...
        print(
            "This can be set to whatever you want the bot to say, with {{Name}} and {{Game}} as placeholders."
        )
        sys.exit()
    ...
    try:
        ...
        print(
            "Description not found in Discord section of config file. Please set Description under [Discord]"
            + " in config.ini"
        )
        print(
            "This can be set to whatever you want to appear under the stream title, with {{Name}} and {{Game}}"
            + " as placeholders."
        )
        sys.exit()
...
def main():
    ...
    while len(twitch_json["data"]) == 0:
        ...
        while request_status == 401:
            twitch_request = requests.get(
                stream_api_url, headers=twitch_headers, params=twitch_params
            )
            request_status = twitch_request.status_code
            if request_status == 401:
                authorize()
                twitch_headers["Authorization"] = (
                    "Bearer " + twitch_app_token_json["access_token"]
                )
                continue
        ...
        if len(twitch_json["data"]) == 1:
            ...
            while request_status == 401:
                game_request = requests.get(
                    game_search_url, headers=twitch_headers, params=game_params
                )
            ...
            while request_status == 401:
                user_request = requests.get(
                    user_search_url, headers=twitch_headers, params=user_params
                )
            ...
            while status_code != 204:
                ...
                if discord_request.status_code == 204:
                    print(
                        "Successfully called Discord API. Waiting 5 seconds to terminate..."
                    )
                    time.sleep(5)
                else:
