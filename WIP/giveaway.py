...
class Giveaway(commands.Cog):
    ...
    async def start_giveaway(
        ...
        any messages from that user in the last (limit) messages will be deleted.
        Checks:
            has_permissions(manage_messages):
            Whether or not the invoking user can manage messages.
            bot_has_permissions(manage_messages, read_message_history):
            Whether or not the bot can manage messages
                and read the message history of a channel,
                both of which are required for the deletion of messages.
        Parameters:
            ctx (commands.Context): The invocation context.
        ...
        if channel:
            ...
            embed.add_field(
                name="End Date",
                value=time.strftime("%I:%M %p on %A, %B %d, %Y"),
                inline=False,
            )
